    ...
    devServer: {
      ...
      proxy: {
        '/api': {
          ...
          router: function(req) {
            if (req.headers['X-Forwarded-Host'] || req.headers['x-forwarded-host'] ) {
              const proto = req.headers['X-Forwarded-Proto'] || req.headers['x-forwarded-proto']
              return {
                protocol: proto.includes(':') ? proto : (proto + ':'),
                host: req.headers['X-Forwarded-Host'] || req.headers['x-forwarded-host'],
                port: req.headers['X-Forwarded-Port'] || req.headers['x-forwarded-port']
              }
            }
          },
        }
